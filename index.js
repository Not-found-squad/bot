    ...
    try {
        ...
        switch (command) {
            ...
            case 'ytplay':
                ...
                caption = `\`❖ Description    :\` *${anu.result.description}*\n`
                caption = `\`❖ Publish date    :\` *${anu.result.publishDate}*\n`
                caption = `\`❖ Url    :\` *${anu.result.url}*\n`
                await lol.replyWithPhoto({ url: anu.result.thumbnail }, { caption: caption, parse_mode: "Markdown" })
                await lol.replyWithAudio({ url: anu.result.url_audio }, { title: anu.result.title, thumb: anu.result.thumbnail })
                await lol.replyWithVideo({ url: anu.result.url_video }, { title: anu.result.title, thumb: anu.result.thumbnail })
                break
            case 'ytmp3':
                if (args.length == 0) return reply(`Example: ${prefix + command} https://www.youtube.com/watch?v=qZIQAk-BUEc`)
                ini_link = args[0]
                anu = await fetchJson(`https://api.zeks.xyz/api/ytmp3?url=${ini_link}&apikey=apivinz`)
                caption = `\`❖ Title    :\` *${anu.result.title}*\n`
                await lol.replyWithPhoto({ url: anu.result.thumbnail }, { caption: caption, parse_mode: "Markdown" })
                await lol.replyWithAudio({ url: anu.result.url_audio }, { title: anu.result.title, thumb: anu.result.thumbnail })
                break
             case 'ig':
                ...
                ini_link = args[0]
                anu = await fetchJson(`https://api.zeks.xyz/api/ig?url=${ini_link}&apikey=apivinz`)
                teks = `${anu.result.caption}`
                await lol.replyWithVideo({url: anu.result[0].url }, {caption: teks})
                break
            case 'ytmp4':
                if (args.length == 0) return reply(`Example: ${prefix + command} https://www.youtube.com/watch?v=qZIQAk-BUEc`)
                ini_link = args[0]
                anu = await fetchJson(`https://api.zeks.xyz/api/ytmp4?url=${ini_link}&apikey=apivinz`)
                caption = `\`❖ Title    :\` *${anu.result.title}*\n`
                await lol.replyWithPhoto({ url: anu.result.thumbnail }, { caption: caption, parse_mode: "Markdown" })
                await lol.replyWithVideo({ url: anu.result.url_video }, { title: anu.result.title, thumb: anu.result.thumbnail })
                break
            ...
            case 'trap':
                ...
                await lol.replyWithPhoto({ url: anu.url })
                break
             case 'yuri':
                ...
                break
             case 'nsfwneko':
                anu = await fetchJson(`https://api.computerfreaker.cf/v1/nsfwneko`)
                await lol.replyWithPhoto({ url: anu.url })
                break
            case 'blowjob':
           ...
           case 'waifu':
                ...
                teks `name : ${anu.name} \n desc : ${anu.desc}`
                await lol.replyWithPhoto({ url: anu.image }, {caption: teks})
                break
            case 'hentai':
            ...
            case 'character':
                ...
                query = args.join(" ")
                anu = await fetchJson(`https://iwi-negev.herokuapp.com/api/anime/chara?search=${query}&apikey=GFL`)
                text = `Id : ${anu.result[0].id}\n`
                text += `Name : ${anu.result[0].name}\n`
                text += `Gender : ${anu.result[0].gender}\n`
                text += `Desc : ${anu.result[0].desc}\n`
                text += `Anime : ${anu.result[0].anime_name}\n`
                await lol.replyWithPhoto({ url: anu.result[0].character_image }, { caption: text })
                break
            case 'manga':
            ...
            case 'trash':
             ...
             case 'trash2':
                if (isQuotedImage || isQuotedAnimation || isQuotedVideo || isQuotedDocument) {
                    file_id = await getFileID()
                    url_file = await tele.getLink(file_id)
                    anu = await fetchJson(`https://nekobot.xyz/api/imagegen?type=trash&url=${url_file}`)      
                    await lol.replyWithPhoto({ url: anu.message })
                } else {
                    reply(`Tag gambar yang sudah dikirim`)
                }
                break
             case 'pensil':
                if (isQuotedImage || isQuotedAnimation || isQuotedVideo || isQuotedDocument) {
                    file_id = await getFileID()
                    url_file = await tele.getLink(file_id)
                    await lol.replyWithPhoto({ url: `https://videfikri.com/api/textmaker/pencil/?urlgbr=${url_file}` })
                } else {
                    reply(`Tag gambar yang sudah dikirim`)
                }
                break
            ...
            case 'graffiti':
             ...
             case 'kanna':
                if (args.length == 0) return reply(`Example: ${prefix + command} Ling`)
                ling = args[0]
                anu = await fetchJson(`https://nekobot.xyz/api/imagegen?type=kannagen&text=${ling}&raw=7`)      
                await lol.replyWithPhoto({ url: anu.message })
                break
             case 'cmm':
                if (args.length == 0) return reply(`Example: ${prefix + command} change my mind`)
                ling = args[0]
                anu = await fetchJson(`https://nekobot.xyz/api/imagegen?type=changemymind&text=${ling}&raw=7`)      
                await lol.replyWithPhoto({ url: anu.message })
                break
            case 'trumptweet':
                if (args.length == 0) return reply(`Example: ${prefix + command} trumptweet`)
                ling = args[0]
                anu = await fetchJson(`https://nekobot.xyz/api/imagegen?type=trumptweet&text=${ling}&raw=7`)      
                await lol.replyWithPhoto({ url: anu.message })
                break
            case 'kusonime':
